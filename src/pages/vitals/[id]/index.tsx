...
function VitalsForm({ visitId }: {visitId:number}) {
  ...
  const onSubmit = (data:CreateVitalsInput) => {
    updateVitalsMutation.mutate({
      ...
      diabetesMellitus: data.diabetesMellitus === "true" ? true : data.diabetesMellitus === "false" ? false : undefined,
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <section className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <RHFInput name="temperature" label="Body Temperature (°C)" type="number" />
          </div>
        </section>
        ...
        <section className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <RHFInput name="systolic" label="Systiolic Blood Pressure (mmHg)" type="number" />
          <RHFInput name="diastolic" label="Diastolic Blood Pressure (mmHg)" type="number" />
        ...
        <section className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <RHFInput name="bloodGlucoseFasting" label="Fasting Blood Glucose (mmol/L)" type="number" />
          <RHFInput name="bloodGlucoseNonFasting" label="Non-Fasting Blood Glucose (mmol/L)" type="number" />
          <RHFInput name="hba1c" label="HbA1c Level (%)" type="number" />
          <RHFInput name="hemocueCount" label="Hemocue Hemoglobin Count (g/dL)" type="number" />
          </div>
          <div className="mt-4">
            <RHFRadio name="diabetesMellitus" label="Diabetes Metiitus History Status Flag"
              radioOptions={[
        ...
        <section className="space-y-4">
          <div className="space-y-4">
            <RHFTextArea name="urineTest" label="Urinalysis Diagnostics (Leukocytes, Nitrites, Protein notes)" rows={3} />
            <RHFTextArea name="others" label="Additional Clinical Remarks" rows={3} />
          </div>
        </section>

        <div className="flex justify-end pt-4 border-t border-slate-100">
          <div className="w-full md:w-44">
              <Button type="submit" title="Save Records" colour="white" variant="filled" loading={updateVitalsMutation.isPending}
                />
          </div>
        </div>
      </form>
