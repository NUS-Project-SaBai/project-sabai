...
excerpt: How Drizzle ORM is configured and used in this project
permalink: /docs/drizzle-orm/
---
...
│       └── schema/            # Table definitions & types (per-domain files)
│           ├── index.ts       # Re-exports every domain file
│           ├── auth.ts
│           ├── villageCodes.ts
│           ├── patients.ts
│           ├── vitals.ts
...
| `src/db/schema/`       | Per-domain files defining tables, columns, relations, and exported types; `index.ts` re-exports them all |
...
export default defineConfig({
  schema: "./src/db/schema", // Directory of per-domain schema files
...
│        │                                                    │
│        ▼                                                    │
│  5. Commit schema files + migration files                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
const results = await db.query.villageCodes.findMany({
  where: and(
    ...
    or(like(villageCodes.name, "%Village%"), gt(villageCodes.id, 10)),
  ),
});
```
